...
Scheduler sc;
// 定义一个天气更新任务
Task tWeatherUpdate(1000 * 60 * 60, TASK_FOREVER, &initWeather, &sc);
// 定义一个时间更新任务
Task tTimeUpdateTask(200, TASK_FOREVER, &ttimeUpdate, &sc);
// 定义一个时间初始化任务
Task ttimeInitTask(TASK_SECOND, TASK_ONCE, &initTimeClient, &sc);
// 定义一个天气展示任务
Task tweatherDisplay(1000, TASK_FOREVER, &displayWeather, &sc);
// 定义一个WiFi连接任务
Task tWifiConnect(1000, TASK_FOREVER, &initWifi, &sc);
// 定义一个文件系统初始化任务
Task tLittleFs(TASK_IMMEDIATE, TASK_ONCE, &initLitteFs, &sc);
// 定义一个获取经纬的任务
Task tLonAndLat(TASK_IMMEDIATE, TASK_ONCE, &getPublicIp, &sc);
...
void setup()
{
   ...
   tLittleFs.enable();
   tWifiConnect.waitFor(tLittleFs.getInternalStatusRequest(), 1000UL, -1);
   ttimeInitTask.waitFor(tWifiConnect.getInternalStatusRequest(), TASK_SECOND, TASK_ONCE);
   tWeatherUpdate.waitFor(tWifiConnect.getInternalStatusRequest(), TASK_HOUR, -1);
   tTimeUpdateTask.waitFor(ttimeInitTask.getInternalStatusRequest(), TASK_MILLISECOND, -1);
   tweatherDisplay.waitFor(tWeatherUpdate.getInternalStatusRequest(), TASK_SECOND, -1);
}
void loop()
{
   server.handleClient();
...
void initLitteFs()
{
   ...
   if (LittleFS.begin())
   {
      ...
      {
         // 不存在WiFi文件
         tLittleFs.getInternalStatusRequest()->signal(-1);
   ...
   tLittleFs.getInternalStatusRequest()->signalComplete();
}
...
void initWifi()
{
   ...
   tWifiConnect.yield(&connectionCheck);
}
...
void connectionCheck()
{
   ...
   if (WiFi.status() == WL_CONNECTED)
   {
      ...
      delay(1000);
      tft_Clear_Bottom();
      tWifiConnect.getStatusRequest()->signalComplete();
      tWifiConnect.disable();
   }
   else // 没有连接成功，重试
   {

      if (tWifiConnect.getRunCounter() % 5 == 0) // 每5秒重试一次
      {
         Serial.println("重试中..");
         WiFi.reconnect();
         yield();
      ...
      if (tWifiConnect.getRunCounter() == 30) // 当到了20秒后，不再重试
      {
         Serial.println("重试了30次了");
         tft_Print_Bottom("Connect" + ssid + "Failed");
         tWifiConnect.getInternalStatusRequest()->signal(-1);
         tWifiConnect.disable();
      }
   }
}
...
void displayWeather()
{
   if (tweatherDisplay.isFirstIteration())
   {
      Serial.println("展示天气");
   }
...
void initWeather()
{
   ...
   if (https.begin(*client, url))
   {
      ...
      if (code == HTTP_CODE_OK || code == HTTP_CODE_MOVED_PERMANENTLY)
      {
         ...
         tWeatherUpdate.getInternalStatusRequest()->signalComplete();
      }
      else
      {
         Serial.println("连接失败" + code);
         tft_Print_Bottom("Initial Weather Failed!");
         tWeatherUpdate.getInternalStatusRequest()->signal(-1);
      }
   }
   else
   {
