...
void setup()
{
    ...
    tWeatherUpdateTask.waitFor(tWifiConnectTask.getInternalStatusRequest(), TASK_SECOND, -1);
    // 初始化时间更新
    tTimeInitTask.waitForDelayed(tWifiConnectTask.getInternalStatusRequest(), TASK_IMMEDIATE, TASK_ONCE);

    tTimeUpdateTask.waitForDelayed(tTimeInitTask.getInternalStatusRequest(), 200, -1);
...
void initWeather()
{
    ...
    if (!WiFi.isConnected())
    {
        ...
        tWeatherUpdateTask.getInternalStatusRequest()->signal()
        return;
    }
