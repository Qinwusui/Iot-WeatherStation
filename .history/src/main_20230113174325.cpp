...
void setup()
{
   ...
   // 初始化Wifi连接
   tWifiConnectTask.waitFor(tInitLittleFsTask.getInternalStatusRequest(), TASK_IMMEDIATE, TASK_ONCE);
   // 初始化时间更新
...
void handleConfigWifi()
{
   ...
   else
   {
      ...
      serializeJsonPretty(json, s);
      server.send(200, "text/javascript", s);
      if (tWifiConnectTask.isEnabled())
      {
         Serial.println("WifiConnect Disabled");
