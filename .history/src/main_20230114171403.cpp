...
void setup()
{
   ...
   // 初始化时间更新
   tTimeInitTask.waitForDelayed(tWifiConnectTask.getInternalStatusRequest(), TASK_SECOND, TASK_ONCE);
   tTimeUpdateTask.waitForDelayed(tTimeInitTask.getInternalStatusRequest(), TASK_SECOND, -1);
   // 初始化天气更新
   tWeatherUpdateTask.waitForDelayed(tWifiConnectTask.getInternalStatusRequest(), TASK_HOUR, -1);
   tWeatherDisplayTask.waitForDelayed(tWeatherUpdateTask.getInternalStatusRequest(), TASK_SECOND, -1);
}
void loop()
{
   sc.execute();
...
void initServer()
{
   ...
   server.on("/config/location", HTTP_GET, handleResetLocation);
   server.begin();
}
void handleWifiList(AsyncWebServerRequest *req)
{
   Serial.println("开始扫描");
   int8_t n=WiFi.scanNetworks(true);
   // delay(2000);
   DynamicJsonDocument json(80 * n);
   json["code"] = 0;
   json["msg"] = "获取成功";
