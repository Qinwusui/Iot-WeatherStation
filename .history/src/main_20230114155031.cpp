...
// 定义一个WiFi扫描结束后任务
Task tWifiScanFinishedTask(&wifiScanListener, &sc);
// 屏幕驱动
...
void setup()
{
   ...
   tWifiScanFinishedTask.waitFor(tWifiScanTask.getInternalStatusRequest(), TASK_IMMEDIATE, TASK_ONCE);
}
void loop()
{
   server.handleClient();
