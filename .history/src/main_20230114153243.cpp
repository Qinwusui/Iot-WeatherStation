...
// 定义一个WiFi扫描任务
Task tWifiScanTask(3000,TASK_FOREVER,&wifiScanner, &sc);
// 定义一个WiFi扫描结束后任务
Task tWifiScanFinishedTask(3000, TASK_FOREVER, &wifiScanListener, &sc);
// 屏幕驱动
...
void wifiScanListener()
{
   if (tWifiScanTask.getRunCounter() % 3 == 0)
   {
      ...
      if (n >= 0)
      {
         ...
         {
            Serial.printf("%s:通道:%d,强度:%d,加密类型:%d\n",
                          WiFi.SSID(i).c_str(),
                          WiFi.channel(i),
                          WiFi.RSSI(i),
         ...
         WiFi.scanDelete();
         tWifiScanTask.getInternalStatusRequest()->signalComplete();
      }
      else
      {
         Serial.println("没有找到网络");
         tWifiScanTask.getInternalStatusRequest()->signal(-1);
      }
   }
}
