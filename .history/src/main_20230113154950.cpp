...
void setup()
{
   ...
   // 初始化Ws服务器
   tInitWebServerTask.waitFor(tWifiConnectTask.getInternalStatusRequest(), TASK_IMMEDIATE, TASK_ONCE);
}
void loop()
{
   server.handleClient();
...
void webSocketEvent(uint8_t num, WStype_t type, uint8_t *payload, size_t length)
{
   ...
   case WStype_DISCONNECTED:
      tft_Print_Bottom("WsClient Disconnected!");
      break;
   case WStype_CONNECTED:
   {
      Serial.println("Ws客户端已连接");
      IPAddress address = wsServer.remoteIP(num);
      char ip[20];
      sprintf(ip, "%d.%d.%d.%d", address[0], address[1], address[2], address[3]);
      tft_Print_Bottom("WsClient" + String(ip) + " Connected!");
      delay(2000);
   }
   break;
   case WStype_TEXT:
...
void handleRoot()
{
   ESPData espData = ESPData();
   espData.code = 0;
   espData.time = millis();
   espData.msg = "Wellcome!";
   String s;
   serializeJsonPretty(espData,s);
...
void initWebSocketServer()
{
   ...
   tInitWebServerTask.getInternalStatusRequest()->signalComplete();
}
