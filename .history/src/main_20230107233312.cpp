...
void setup()
{
   ...
   tweatherUpdateTask.waitFor(tWifiConnect.getInternalStatusRequest(), TASK_HOUR, -1);
   ttimeUpdateTask.waitFor(ttimeInitTask.getInternalStatusRequest(), TASK_SECOND, -1);
   tweatherDisplay.waitFor(tweatherUpdateTask.getInternalStatusRequest(), TASK_SECOND, -1);
}
void loop()
{
   server.handleClient();
...
void connectionCheck()
{
   ...
   {
      Serial.println("连接成功");
      getPublicIp(!LittleFS.exists(locFile));
...
void startWebSocket()
{
   tft_Print_Bottom("Init WebSocketClient");
...
void initWeather()
{
   ...
   if (https.begin(*client, url))
   {
      ...
      else
      {
         ...
         tweatherUpdateTask.getInternalStatusRequest().sign
