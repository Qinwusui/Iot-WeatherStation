...
void setup()
{
    ...
    // 初始化时间更新
    tTimeInitTask.waitFor(tWifiConnectTask.getInternalStatusRequest(), TASK_IMMEDIATE, TASK_ONCE);

    tTimeUpdateTask.waitForDelayed(tWifiConnectTask.getInternalStatusRequest(), 200, -1);
...
void initWeather()
{
    ...
    tWeatherUpdateTask.yield(&checkWeather);
}
void checkWeather()
{
    if (wa.humidity != NULL)
