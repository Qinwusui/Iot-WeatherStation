...
void setup()
{
    ...
    // 初始化时间更新
    tTimeInitTask.waitForDelayed(tWifiConnectTask.getInternalStatusRequest(), TASK_IMMEDIATE, TASK_ONCE);

    tTimeUpdateTask.waitForDelayed(tTimeInitTask.getInternalStatusRequest(), 1000, -1);
    tWeatherDisplayTask.waitFor(tWeatherUpdateTask.getInternalStatusRequest(), TASK_IMMEDIATE, -1);
}
// 循环函数
void loop()
{
...
void ttimeUpdate() // 1s更新一次
{
    ...
    {
        Serial.println("更新时间失败");
        tTimeUpdateTask.getInternalStatusRequest()->signal(-1);
    }
    ...
    String time = hour >10  ? String(hour) : "0" + String(hour) + ":" +min<10? String(min):"0"+String(min) + ":" + String(second);
...
void initWeather()
{
    ...
    if (wa.code != "200")
    {
        ...
        if (tWeatherUpdateTask.getRunCounter() % 5 == 0)
        {
            tWeatherUpdateTask.enable();
        }
        else
        {
            if (tWeatherUpdateTask.getRunCounter() == 20)
            {
                tWeatherUpdateTask.getInternalStatusRequest()->signal(-1);
